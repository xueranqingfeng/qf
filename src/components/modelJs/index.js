...
function analysisObject(n) {
    ...
    if (n instanceof Model) {
        ...
        let isNotSettingKeys = keys.some(item => ['type', 'default', 'unit', 'format', 'parse', 'dispose', 'computed', 'reName'].indexOf(item) == -1);
    ...
    switch (model.type) {
                ...
                for (const i of keys) {
                    if (model.value.hasOwnProperty(i)) {
                        ...
                        if (param.removeNull && (_out == undefined || _out == null || (Utils.isArray(_out) && _out.length == 0))) {
                            ...
                            if (model.value[i].reName) {
                                outData[model.value[i].reName] = _out
                                delete outData[model.value[i]]
                            } else {
                                outData[i] = _out;
                            }
                        }
                ...
                for (const i of Object.keys(data)) {
                    if (model.value.hasOwnProperty(i)) {
                        ...
                        if (d != undefined && d != null) {
                            ...
                            if (model.value[i].reName) {
                                outData[model.value[i].reName] = d
                            } else {
                                outData[i] = d;
                            }
                        }
        ...
        case TYPE.NUMBER:
            if (Utils.isString(data) && data == '') {
                ...
                outData = Number(data);
                if (model.unit) {
                    if (model.format == 'except') {
                        outData = Utils.div(outData, model.unit);
                    } else if (model.format == 'ride') {
...
class Model {
    ...
    parse(data, param = { isParse: true, isDispose: false }) {
        return _parse(data, this._model, Utils.extend({}, defaultParam, param));
    }

    dispose(data, param = { isParse: false, isDispose: true }) {
        return _parse(data, this._model, Utils.extend({}, defaultParam, param));
    }
}
...
Model.disposeDateFormat = (str, format) => {
    return manba(str).format(format);
}
Model.config = (params) => {
